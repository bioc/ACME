...
Now, \Robject{a} is an R data structure (of class \Rclass{aGFF}) that contains the data from two test GFF files.
...
\subsection{Generating files for the Affymetrix Integrated Genome Browser}
